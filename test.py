...
async def send():
    async for _ in arange(1000):
        i = random.randint(0, 9)
        c = pk[i]
        priv = sk[i]
        d = pk[random.randint(0, 9)]
        a = random.randint(100, 110)
        t = int(round(time.time() * 1000))
        m = str(random.randint(0, 100000))
        ...
        print(tx)
        r = await client.post('https://api.yatmoney.ru/send/', data=json.dumps(tx))
        print(r.text)

async def main():
    await asyncio.gather(*[send() for i in range(50)])
